...
function createLPTPool(maxConcurrency) {
	...
	const executeTask = async (taskFunction) => {
		...
		if (activeCount >= maxConcurrency) {
			console.log(taskFunction.fnName, ': 加入等待队列，当前并发数:', activeCount);
		...
		console.log(taskFunction.fnName, ': 准备执行，当前并发数:', activeCount);
		// 增加当前的并发数
		activeCount++;
		try {
			// 执行任务
			...
			console.log(taskFunction.fnName, ': 执行完毕');
			// 任务执行完毕，减少当前的并发数
			activeCount--;
			// 如果等待队列中有任务，则执行下一个任务
			if (waitingQueue.length) {
				const nextTask = waitingQueue.shift();
				console.log(nextTask.fnName, ': 从等待队列中取出，准备执行，当前并发数:', activeCount);
				nextTask.resolve();
			}
		}
	};
